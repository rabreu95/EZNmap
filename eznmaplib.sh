...
mal() {
    nmap -p80 -Pn -T$speed --script http-google-malware $host 
}
os() {
    nmap -A -Pn -T$speed $host 
}
syn() {
...
http-discov() {
    nmap --script http-enum -Pn -sV -v -T$speed $host 
}

basic-xss() {
    nmap -sV -Pn --script -T$speed -v http-unsafe-output-escaping $host 
}

basic-sql-inject() {
    nmap -sV -Pn -T$speed -v --script http-sql-injection $host 
}

basic-wp-bf() {
    nmap -sV -Pn -T$speed -v --script http-wordpress-brute $host 
}
...
ftp-bf(){
    nmap -Pn --script ftp-brute -v -p 21 -T$speed $host 
}
